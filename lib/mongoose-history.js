...
module.exports = function historyPlugin(schema, options) {
  ...
  const skip_when_field_present = options && options.skip_when_field_present;
  ...
  function saveHistoryModel(original, d, historyDoc, collectionName, next) {
    // Prevent history object from being created if given field is present
    if (!d[skip_when_field_present] && !d.force_history_skip){
      if (metadata) {
